...
```
        ...
        ║                                            ║
        ║  ┌──────────┐ ┌────────┐ ┌──────────────┐ ║
        ║  │ agent.py │ │memory  │ │ mcp_debug    │ ║
        ║  │ (核心)   │ │ .py    │ │ .py (日志)   │ ║
        ║  └──────────┘ └────────┘ └──────────────┘ ║
        ║                                            ║
        ║  提供路线规划、位置查询、多轮对话、长期记忆  ║
        ╚════════════════════════════════════════════╝
          │              │              │
          │              │              │
          ▼              ▼              ▼
┌─────────────┐  ┌──────────────┐  ┌──────────────────┐
│  LLM API    │  │  高德地图     │  │  华为云 Memory   │
│  (MiniMax / │  │  MCP Server  │  │  SDK             │
│   OpenAI)   │  │              │  │                  │
│             │  │  - 路线规划   │  │  - 长期记忆存储   │
│  - 自然语言 │  │  - POI 搜索   │  │  - 语义检索      │
│    理解生成  │  │  - 地理编码   │  │  - Session 管理  │
│  - 工具调用 │  │  - 天气查询   │  │                  │
│    决策     │  │  - 距离测量   │  │                  │
└─────────────┘  └──────────────┘  └──────────────────┘
```
...
| 要素 | 类型 | 交互方式 | 说明 |
|------|------|----------|------|
| **用户/调用方** | 外部角色 | HTTP POST JSON | 通过 `agentarts invoke` CLI 或 curl 发起请求 |
| **NavAgent** | 本系统 | — | 基于 AgentArts Runtime（FastAPI + Uvicorn :8080） |
| **LLM API** | 外部系统 | OpenAI 兼容 API | MiniMax-M2.5 / GPT-4o-mini，负责推理和工具调用决策 |
| **高德地图 MCP** | 外部系统 | MCP over HTTP (JSON-RPC 2.0) | 15 种地图工具，Streamable HTTP 传输 |
| **华为云 Memory** | 外部系统 | REST API | 按 actor_id 维度存储/检索用户长期记忆 |
| **AgentArts 平台** | 运行环境 | IAM 认证 | 提供部署、运行时、网关、可观测性 |
